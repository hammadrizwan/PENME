...
class CounterFactDataset(Dataset):
    def __init__(self, json_file_path):
        with open(json_file_path, 'r') as jsonl_file:
          lines = jsonl_file.readlines()[0:2000]
        # print(lines)
...
def get_model(PATH: str,access_token:str,device ):
    ...
    if(PATH is None):
        model = LlamaModel.from_pretrained("meta-llama/Llama-2-7b-hf",access_token=access_token)
        tokenizer = LlamaTokenizer.from_pretrained("meta-llama/Llama-2-7b-hf",access_token=access_token)
    else:
...
if __name__ == '__main__':
    ...
    access_token=None # access token for huggingface is needed
