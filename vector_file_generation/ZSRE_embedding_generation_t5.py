            ...
            with torch.no_grad():
                for batch in tqdm(data_loader, desc="Processing batches", leave=False):
                    ...
                    for string in data_entry["neighbours_test_question"]:
                        ...
                        outputs = model(inputs, decoder_input_ids=inputs, output_hidden_states=True)
                        data_entry["vectors_neighbours_test"].append([ret[layer_fc1_vals].output[:,-1, :] for layer_fc1_vals in ret][0][0].detach().cpu().numpy().tolist())
                    ...
                    for string in data_entry["neighbours_train_question"]:
                        ...
                        inputs=tokenizer(string, return_tensors="pt")["input_ids"].to(device)    
                        outputs = model(inputs, decoder_input_ids=inputs, output_hidden_states=True)
                        data_entry["vectors_neighbours_train"].append([ret[layer_fc1_vals].output[:,-1, :] for layer_fc1_vals in ret][0][0].detach().cpu().numpy().tolist())
                    ...
                    for string in data_entry["paraphrases_train"]:
                        ...
                        inputs=tokenizer(string, return_tensors="pt")["input_ids"].to(device)
                        outputs = model(inputs, decoder_input_ids=inputs, output_hidden_states=True)
                        data_entry["vectors_paraphrases_train"].append([ret[layer_fc1_vals].output[:,-1, :] for layer_fc1_vals in ret][0][0].detach().cpu().numpy().tolist())
                    ...
                    for string in data_entry["paraphrases_test"]:
                        ...
                        inputs=tokenizer(string, return_tensors="pt")["input_ids"].to(device)
                        outputs = model(inputs, decoder_input_ids=inputs, output_hidden_states=True)
                        data_entry["vectors_paraphrases_test"].append([ret[layer_fc1_vals].output[:,-1, :] for layer_fc1_vals in ret][0][0].detach().cpu().numpy().tolist())
...
if __name__ == '__main__':
    ...
    model_path=None#add local path if you want to use local model
   
    model,tokenizer=get_model(model_path,device)
    
    file_path=""#add path to zsre dataset
    zsre_dataset=ZSRE(file_path,tokenizer)
    data_loader = DataLoader(zsre_dataset, batch_size=1, shuffle=False)
    file_save_path=""#save path for zsre dataset with embeddings
    get_embedding_dataset_special_token(file_path,model,tokenizer,zsre_dataset,file_save_path,device)
